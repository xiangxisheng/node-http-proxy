module.exports = (oFun, config, httpsrv_res, httpsrv_req, httpreq_res) => {
    if (!config.sys.http.limit.enabled) return false;
    ...
    if (resLen > config.sys.http.limit.max_size_byte) {
        ...
        oFun.http.endmsg.sizeLimited(httpsrv_res, resLen, config.sys.http.limit.max_size_byte);
        return true;//达到限制
    }
    return false;//不限制
};
